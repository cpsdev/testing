    ...
    case SPINDLE_PRIMARY: // main spindle
      if (true) {
        ...
        if (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
          ...
          sOutput.reset();
          writeBlock(getCode("CONSTANT_SURFACE_SPEED_ON"), sOutput.format(tool.surfaceSpeed * ((unit == MM) ? 1/1000.0 : 1/12.0)), getCode(tool.clockwise ? "START_MAIN_SPINDLE_CW" : "START_MAIN_SPINDLE_CCW"));
          sOutput.reset();
        } else {
          if (tool.spindleRPM < 1) {
            error(localize("Spindle speed out of range."));
          ...
          writeBlock(getCode("CONSTANT_SURFACE_SPEED_OFF"), sOutput.format(tool.spindleRPM), getCode(tool.clockwise ? "START_MAIN_SPINDLE_CW" : "START_MAIN_SPINDLE_CCW"));
        }
    ...
    case SPINDLE_SECONDARY: // sub spindle
      ...
      if (true) {
        ...
        if (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
          ...
          pOutput.reset();
          writeBlock(getCode("CONSTANT_SURFACE_SPEED_ON"), pOutput.format(tool.surfaceSpeed * ((unit == MM) ? 1/1000.0 : 1/12.0)), getCode(tool.clockwise ? "START_SUB_SPINDLE_CW" : "START_SUB_SPINDLE_CCW"));
          pOutput.reset();
        } else {
          if (tool.spindleRPM < 1) {
            error(localize("Spindle speed out of range."));
          ...
          writeBlock(getCode("CONSTANT_SURFACE_SPEED_OFF"), pOutput.format(tool.spindleRPM), getCode(tool.clockwise ? "START_SUB_SPINDLE_CW" : "START_SUB_SPINDLE_CCW"));
        }
        // wait for spindle here if required
      }
      break;
